...
export function setupAuth(app: Express) {
  ...
  const sessionSettings: session.SessionOptions = {
    secret: process.env.SESSION_SECRET || "fiscavo-secret-key-change-in-production",
    resave: false,
    ...
    new LocalStrategy(async (username, password, done) => {
      try {
        ...
        if (!user || !(await comparePasswords(password, user.password))) {
          return done(null, false, { message: "Onjuiste gebruikersnaam of wachtwoord" });
        } else {
  ...
  app.post("/api/login", (req, res, next) => {
    passport.authenticate("local", (err: any, user: any, info: any) => {
      ...
      if (!user) {
        return res.status(401).json({ message: info?.message || "Onjuiste inloggegevens" });
      }
      req.login(user, (err) => {
        if (err) {
